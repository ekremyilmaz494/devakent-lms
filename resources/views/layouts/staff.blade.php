...
<head>
    ...
    <title>@yield('title', 'Eğitim Portalı') — {{ config('app.name') }}</title>
...
<body class="font-sans antialiased bg-gray-50 dark:bg-gray-900">
    <div x-data="{ sidebarOpen: false }" class="flex h-screen overflow-hidden">
        ...
        {{-- Sidebar: mobilde gizli, lg'de sabit --}}
        ...
        <div class="flex flex-col flex-1 overflow-hidden w-full">
            ...
            <main class="flex-1 overflow-y-auto p-4 md:p-6">
                @if(session('success'))
                    <div class="mb-4 flex items-center gap-3 p-3 bg-emerald-100 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-300 rounded-xl text-sm"
                ...
                @if(session('error'))
                    <div class="mb-4 flex items-center gap-3 p-3 bg-red-100 dark:bg-red-900/30 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-300 rounded-xl text-sm"
                         x-data="{ show: true }" x-show="show"
                         x-transition:enter="transition ease-out duration-300"
                         x-transition:enter-start="opacity-0 -translate-y-2"
                         x-transition:enter-end="opacity-100 translate-y-0"
