<div>
    ...
    <div class="flex items-center justify-between mb-6">
        <div class="relative w-72">
            <input wire:model.live.debounce.300ms="search" type="text" placeholder="{{ __('lms.search_department') }}"
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
        <table class="w-full text-sm">
            <thead class="bg-gray-50 dark:bg-gray-700">
                <tr>
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('lms.department_name') }}</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('lms.department_desc') }}</th>
                    <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('lms.staff_count') }}</th>
                    <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('lms.department_is_active') }}</th>
                    <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('lms.actions') }}</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                @forelse($departments as $dept)
                <tr wire:key="dept-{{ $dept->id }}" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                    ...
                    <td class="px-6 py-4 text-center">
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $dept->is_active ? 'bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-300' : 'bg-red-100 dark:bg-red-900/50 text-red-700 dark:text-red-300' }}">
                            {{ $dept->is_active ? __('lms.active') : __('lms.inactive') }}
                        </span>
                    </td>
                    <td class="px-6 py-4 text-right space-x-2">
                        <button wire:click="openEdit({{ $dept->id }})" class="text-primary-600 dark:text-primary-400 hover:text-primary-800 dark:hover:text-primary-300 text-sm font-medium">{{ __('lms.edit') }}</button>
                        <button wire:click="confirmDelete({{ $dept->id }})" class="text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300 text-sm font-medium">{{ __('lms.delete') }}</button>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="5" class="px-6 py-12 text-center text-gray-400 dark:text-gray-500">{{ __('lms.department_empty') }}</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 modal-backdrop-enter" wire:click.self="$set('showModal', false)">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md mx-4 p-6 modal-content-enter">
            <h3 class="text-lg font-semibold text-gray-800 dark:text-white mb-4">
                {{ $editingId ? __('lms.edit_department') : __('lms.add_department') }}
            </h3>

            <form wire:submit="save" class="space-y-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('lms.department_name') }}</label>
                    <input wire:model="name" type="text" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm bg-white dark:bg-gray-700 text-gray-800 dark:text-white focus:ring-primary-500 focus:border-primary-500" placeholder="{{ __('lms.department_name_ph') }}">
                    @error('name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('lms.department_desc') }}</label>
                    <textarea wire:model="description" rows="3" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm bg-white dark:bg-gray-700 text-gray-800 dark:text-white focus:ring-primary-500 focus:border-primary-500" placeholder="{{ __('lms.department_desc_ph') }}"></textarea>
                ...
                <div class="flex items-center">
                    ...
                    <label for="is_active" class="ml-2 text-sm text-gray-700 dark:text-gray-300">{{ __('lms.department_is_active') }}</label>
                </div>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 modal-backdrop-enter" wire:click.self="$set('showDeleteModal', false)">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-sm mx-4 p-6 text-center modal-content-enter">
            ...
            <h3 class="text-lg font-semibold text-gray-800 dark:text-white mb-2">{{ __('lms.department_delete_title') }}</h3>
            <p class="text-sm text-gray-500 dark:text-gray-400 mb-6">{{ __('lms.department_delete_body') }}</p>
