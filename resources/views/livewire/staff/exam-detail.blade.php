        ...
        <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[85vh] flex flex-col">
            ...
            <div class="flex-1 overflow-y-auto px-6 py-4 space-y-4">
                @foreach($answers as $i => $answer)
                    <div class="rounded-xl border {{ $answer['is_correct'] ? 'border-emerald-200 dark:border-emerald-800 bg-emerald-50/50 dark:bg-emerald-900/10' : 'border-red-200 dark:border-red-800 bg-red-50/50 dark:bg-red-900/10' }} p-4">
                        <div class="flex items-start gap-3 mb-3">
                            <span class="flex-shrink-0 w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold {{ $answer['is_correct'] ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400' : 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400' }}">{{ $i + 1 }}</span>
                        ...
                        <div class="ml-10 space-y-1.5">
                            @foreach(['a', 'b', 'c', 'd'] as $opt)
                                @php
                                    ...
                                    $isCorrect = $answer['correct_option'] === $opt;
                                @endphp
                                <div class="flex items-center gap-2 px-3 py-1.5 rounded-lg text-[13px]
                                    @if($isCorrect) bg-emerald-100 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 font-medium
                                    @elseif($isSelected && !$isCorrect) bg-red-100 dark:bg-red-900/20 text-red-700 dark:text-red-400 line-through
                                    @else text-gray-600 dark:text-gray-400
                                    @endif">
                                    <span class="w-5 h-5 rounded flex items-center justify-center text-[10px] font-bold flex-shrink-0
                                        @if($isCorrect) bg-emerald-500 text-white
                                        @elseif($isSelected && !$isCorrect) bg-red-500 text-white
