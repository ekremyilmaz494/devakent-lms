<div>
    ...
    @if($activeTab === 'overview')
        <div class="space-y-6">
            ...
            @if(($surveyStats->total_responses ?? 0) > 0)
            <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
                <div class="px-5 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
                    <h3 class="text-sm font-bold text-gray-900 dark:text-white">Geri Bildirim Anketi</h3>
                    <span class="text-xs text-gray-400 dark:text-gray-500">{{ $surveyStats->total_responses }} yanıt</span>
                </div>
                <div class="p-5">
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-5">
                        @foreach([
                            ['label' => 'Genel Memnuniyet', 'val' => $surveyStats->avg_overall],
                            ['label' => 'İçerik Kalitesi', 'val' => $surveyStats->avg_content],
                            ['label' => 'Pratik Fayda', 'val' => $surveyStats->avg_usefulness],
                        ] as $metric)
                        <div>
                            <p class="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">{{ $metric['label'] }}</p>
                            <div class="flex items-center gap-2">
                                <div class="flex items-center gap-0.5">
                                    @for($s = 1; $s <= 5; $s++)
                                    <svg class="w-4 h-4 {{ $s <= round($metric['val']) ? 'text-amber-400' : 'text-gray-200 dark:text-gray-700' }}" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
                                    @endfor
                                </div>
                                <span class="text-sm font-bold text-gray-700 dark:text-gray-300">{{ number_format($metric['val'], 1) }}/5</span>
                            </div>
                        </div>
                        @endforeach
                    </div>
                    @php
                        $dTotal = array_sum($surveyDuration) ?: 1;
                        $dLabels = ['too_short' => 'Çok Kısa', 'appropriate' => 'Uygun', 'too_long' => 'Çok Uzun'];
                    @endphp
                    <div>
                        <p class="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2">Eğitim Süresi Algısı</p>
                        <div class="flex flex-wrap gap-2">
                            @foreach($dLabels as $key => $label)
                            @php $cnt = $surveyDuration[$key] ?? 0; $pct = round($cnt / $dTotal * 100); @endphp
                            <div class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gray-50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-700">
                                <span class="text-xs font-medium text-gray-600 dark:text-gray-400">{{ $label }}</span>
                                <span class="text-xs font-bold text-gray-800 dark:text-white">{{ $cnt }}</span>
                                <span class="text-[10px] text-gray-400 dark:text-gray-500">(%{{ $pct }})</span>
                            </div>
                            @endforeach
                        </div>
                    </div>
                    ...
                    @foreach($videos as $video)
                        ...
                        <div class="rounded-xl border transition-all
                            ...
                            <button wire:click="selectVideo({{ $video->id }})"
                                    class="w-full text-left flex items-start gap-3 p-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="flex items-center gap-2 mt-0.5 flex-wrap">
                                        ...
                                        {{-- Transcode status rozeti --}}
                                        @php $ts = $video->transcode_status ?? 'pending'; @endphp
                                        @if($ts === 'completed')
                                        ...
                                        @elseif($ts === 'processing')
                                            <span class="px-1.5 py-0.5 bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 text-xs font-medium rounded animate-pulse">İşleniyor…</span>
                                        @elseif($ts === 'failed')
                                            <span class="px-1.5 py-0.5 bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 text-xs font-medium rounded">Hata</span>
                                        @else
                                            <span class="px-1.5 py-0.5 bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400 text-xs font-medium rounded">Bekliyor</span>
                                        @endif
                                    </div>
                                </div>
                                @if($selectedVideoId === $video->id)
                                    <svg class="w-4 h-4 text-primary-500 flex-shrink-0 mt-1" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
                                @endif
                            </button>
                            {{-- Retranscode butonu — yalnızca failed/pending için göster --}}
                            @if(in_array($ts, ['failed', 'pending']) && $video->video_path)
                            <div class="px-3 pb-2.5">
                                <button wire:click="retranscodeVideo({{ $video->id }})"
                                        wire:confirm="Bu videoyu yeniden işleme kuyruğuna eklemek istiyor musunuz?"
                                        class="w-full flex items-center justify-center gap-1.5 px-3 py-1.5 text-xs font-medium text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/50 rounded-lg hover:bg-amber-100 dark:hover:bg-amber-900/30 transition-colors">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
                                    {{ $ts === 'failed' ? 'Yeniden Dene' : 'HLS Oluştur' }}
                                </button>
                            </div>
                            @endif
                        </div>
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
                            <div class="flex items-start justify-between gap-3">
                                <div class="min-w-0">
                                    ...
                                    @php $d = $selectedVideo->video_duration_seconds ?? 0; $svTs = $selectedVideo->transcode_status ?? 'pending'; @endphp
                                    <div class="flex items-center gap-3 mt-1.5 flex-wrap">
                                        ...
                                        @elseif($svTs === 'processing')
                                            <span class="inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 animate-pulse">
                                        ...
                                        @elseif($svTs === 'failed')
                                            <span class="inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-full bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400">
                                        ...
                                        @else
                                            <span class="inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-full bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400">Kuyrukta</span>
                                        @endif
                                    </div>
                                </div>
                                {{-- Retranscode butonu --}}
                                @if(in_array($svTs, ['failed', 'pending']) && $selectedVideo->video_path)
                                <button wire:click="retranscodeVideo({{ $selectedVideo->id }})"
                                        wire:confirm="Bu videoyu yeniden işleme kuyruğuna eklemek istiyor musunuz?"
                                        class="flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/50 rounded-lg hover:bg-amber-100 dark:hover:bg-amber-900/30 transition-colors">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
                                    {{ $svTs === 'failed' ? 'Yeniden Dene' : 'HLS Oluştur' }}
                                </button>
                                @endif
                            </div>
                        </div>
    ...
    @if($activeTab === 'results')
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
            ...
            @else
                ...
                @if(isset($resultsPaginator) && $resultsPaginator->hasPages())
                    <div class="px-4 py-3 border-t border-gray-100 dark:border-gray-700">
