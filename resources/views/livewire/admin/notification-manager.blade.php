    ...
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
                ...
                <div class="px-5 py-4 border-b border-gray-200 dark:border-gray-700">
                    <div class="flex items-center gap-2">
                        ...
                        <h3 class="text-sm font-semibold text-gray-900 dark:text-white">{{ __('lms.send_notification') }}</h3>
                    </div>
                </div>
                ...
                <form wire:submit="send" class="p-5 space-y-4">
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1.5">
                            <label class="text-xs font-medium text-gray-700 dark:text-gray-400">{{ __('lms.notification_title_label') }}</label>
                            <span class="text-[11px] text-gray-600">{{ strlen($title) }}/100</span>
                        </div>
                        <input wire:model="title" type="text" maxlength="100" placeholder="{{ __('lms.notification_title_ph') }}"
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1.5">
                            <label class="text-xs font-medium text-gray-700 dark:text-gray-400">{{ __('lms.notification_message_label') }}</label>
                            <span class="text-[11px] text-gray-600">{{ strlen($message) }}/500</span>
                        </div>
                        <textarea wire:model="message" rows="4" maxlength="500" placeholder="{{ __('lms.notification_message_ph') }}"
                    ...
                    {{-- Type --}}
                    <div>
                        <label class="text-xs font-medium text-gray-700 dark:text-gray-400 mb-2 block">{{ __('lms.notification_type') }}</label>
                        <div class="flex gap-2">
                            @foreach(['info' => [__('lms.type_info'), 'bg-blue-100 text-blue-700 border-blue-200 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-800'], 'warning' => [__('lms.type_warning'), 'bg-amber-100 text-amber-700 border-amber-200 dark:bg-amber-900/30 dark:text-amber-400 dark:border-amber-800'], 'success' => [__('lms.type_success'), 'bg-emerald-100 text-emerald-700 border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-400 dark:border-emerald-800']] as $key => $info)
                    ...
                    {{-- Target --}}
                    <div>
                        <label class="text-xs font-medium text-gray-700 dark:text-gray-400 mb-2 block">{{ __('lms.notification_target') }}</label>
                        <div class="space-y-2">
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input wire:model.live="target_type" type="radio" value="all" class="text-primary-600 focus:ring-primary-500" />
                                <span class="text-sm text-gray-700 dark:text-gray-300">{{ __('lms.target_all') }}</span>
                            </label>
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input wire:model.live="target_type" type="radio" value="department" class="text-primary-600 focus:ring-primary-500" />
                                <span class="text-sm text-gray-700 dark:text-gray-300">{{ __('lms.target_department') }}</span>
                            </label>
                        </div>
                        ...
                        @if($target_type === 'department')
                            <select wire:model="target_department_id" class="mt-2 w-full border border-gray-300 dark:border-gray-700 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 focus:ring-primary-500 focus:border-primary-500">
                                <option value="">{{ __('lms.select_department') }}</option>
                    ...
                    <button type="submit" class="w-full px-4 py-2.5 text-sm font-semibold text-white bg-primary-600 rounded-lg hover:bg-primary-700 transition-colors shadow-sm disabled:opacity-50" wire:loading.attr="disabled" wire:target="send">
                        <span wire:loading.remove wire:target="send">{{ __('lms.send_notification') }}</span>
        ...
        <div class="lg:col-span-3">
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden shadow-sm">
                <div class="h-1.5 bg-gradient-to-r from-primary-400 to-primary-600"></div>
                <div class="px-5 py-4 border-b border-gray-200 dark:border-gray-700">
                    <h3 class="text-sm font-semibold text-gray-900 dark:text-white">{{ __('lms.notification_history') }}</h3>
                </div>
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="bg-gray-50/80 dark:bg-gray-700/40 border-b border-gray-200 dark:border-gray-700">
                                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-400 uppercase tracking-wider">{{ __('lms.notifications') }}</th>
                                <th class="px-4 py-3 text-center text-xs font-semibold text-gray-700 dark:text-gray-400 uppercase tracking-wider">{{ __('lms.notification_type') }}</th>
                                <th class="px-4 py-3 text-center text-xs font-semibold text-gray-700 dark:text-gray-400 uppercase tracking-wider">{{ __('lms.notification_target') }}</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-400 uppercase tracking-wider">{{ __('lms.sent_by') }}</th>
                                <th class="px-4 py-3 text-center text-xs font-semibold text-gray-700 dark:text-gray-400 uppercase tracking-wider">{{ __('lms.loading') }}</th>
                            </tr>
                        </thead>
                            ...
                            @forelse($notifications as $notif)
                                @php
                                    ...
                                    $typeLabel = match($notif->type) {
                                        'info' => __('lms.type_info'),
                                        'warning' => __('lms.type_warning'),
                                        'success' => __('lms.type_success'),
                                        default => $notif->type,
                                    };
                                @endphp
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        <span class="text-xs text-gray-600 dark:text-gray-300">
                                            @if($notif->target_type === 'all')
                                                {{ $notif->recipients_count }} {{ __('lms.recipients_count') }}
                                            @else
