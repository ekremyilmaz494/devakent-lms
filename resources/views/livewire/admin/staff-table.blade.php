<div>
    {{-- Flash Notifications --}}
    @if(session('success'))
        <div class="mb-4 flex items-center gap-3 p-3.5 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-xl">
    ...
    @endif
    @if(session('error'))
        <div class="mb-4 flex items-center gap-3 p-3.5 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl">
    ...
    @endif
    @if(session('warning'))
        <div class="mb-4 flex items-center gap-3 p-3.5 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl">
            ...
            <p class="text-sm text-amber-700 dark:text-amber-300 font-medium">{{ session('warning') }}</p>
        </div>
    @endif
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
        <div class="flex items-center gap-3">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ __('lms.staff_title') }}</h2>
            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-primary-100 dark:bg-primary-900/40 text-primary-700 dark:text-primary-300">
                {{ $totalStaff }} {{ __('lms.staff_count_label') }}
            </span>
        </div>
        <div class="flex items-center gap-2 flex-wrap">
            ...
            {{-- Yeni Personel Ekle --}}
            <button wire:click="openCreate" class="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-semibold text-white bg-primary-600 hover:bg-primary-700 rounded-lg transition-colors shadow-sm">
    ...
    <div class="flex flex-col sm:flex-row gap-3 mb-4">
        <div class="relative flex-1 max-w-md">
            ...
            <input wire:model.live.debounce.300ms="search" type="text" placeholder="{{ __('lms.search_staff') }}"
        ...
        <select wire:model.live="filterStatus" class="border border-gray-300 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 pl-3 pr-10 py-2 focus:ring-primary-500 focus:border-primary-500">
            ...
            <option value="1">{{ __('lms.active') }}</option>
            <option value="0">{{ __('lms.inactive') }}</option>
        </select>
    </div>
    ...
    @if(count($selectedStaff) > 0)
        <div class="flex items-center gap-3 mb-4 px-4 py-3 bg-primary-50 dark:bg-primary-900/20 border border-primary-200 dark:border-primary-800 rounded-lg">
            <span class="text-sm font-medium text-primary-700 dark:text-primary-300">{{ __('lms.selected_count', ['count' => count($selectedStaff)]) }}</span>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-gray-50/80 dark:bg-gray-700/40 border-b border-gray-200 dark:border-gray-700">
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-400 uppercase tracking-wider">{{ __('lms.title') }}</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-400 uppercase tracking-wider">{{ __('lms.progress') }}</th>
                    ...
                    @forelse($staff as $user)
                        @php
                            $progressPercent = $user->enrollments_count > 0
                                ...
                                : 0;
                        @endphp
                        <tr wire:key="staff-{{ $user->id }}" class="hover:bg-gray-50/50 dark:hover:bg-gray-700/20 transition-colors {{ in_array($user->id, $selectedStaff) ? 'bg-primary-50/50 dark:bg-primary-900/10' : '' }}">
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3">
                                <span class="text-xs text-gray-600 dark:text-gray-400">
                                    {{ $user->last_login_at ? $user->last_login_at->diffForHumans() : __('lms.last_login_never') }}
                                </span>
                            </td>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-end gap-0.5">
                                    <button wire:click="viewDetail({{ $user->id }})" class="p-1.5 text-gray-400 hover:text-primary-600 hover:bg-primary-50 dark:hover:bg-primary-900/30 rounded-lg transition-colors" title="{{ __('lms.view_detail') }}">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" /><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                                    </button>
                                    <button wire:click="openEdit({{ $user->id }})" class="p-1.5 text-gray-400 hover:text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-900/30 rounded-lg transition-colors" title="{{ __('lms.edit') }}">
                                    ...
                                    <button wire:click="confirmDelete({{ $user->id }})" class="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition-colors" title="{{ __('lms.delete') }}">
                        ...
                        <tr>
                            <td colspan="9" class="px-4 py-16 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-sm font-medium text-gray-600 dark:text-gray-400">{{ __('lms.staff_empty') }}</p>
                                    <p class="text-xs text-gray-600 dark:text-gray-400 mt-1">{{ __('lms.staff_empty_sub') }}</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
                ...
                <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg z-10 border border-gray-200 dark:border-gray-700 modal-content-enter">
                    ...
                    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                        <div class="flex items-center gap-3">
                            ...
                            <h3 class="text-base font-semibold text-gray-900 dark:text-white">
                                {{ $editingId ? __('lms.edit_staff') : __('lms.new_staff') }}
                            </h3>
                        </div>
                    ...
                    <form wire:submit="save" class="px-6 py-5 space-y-4">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-medium text-gray-700 dark:text-gray-400 mb-1.5">{{ __('lms.first_name') }} *</label>
                            ...
                            <div>
                                <label class="block text-xs font-medium text-gray-700 dark:text-gray-400 mb-1.5">{{ __('lms.last_name') }} *</label>
                        ...
                        <div>
                            <label class="block text-xs font-medium text-gray-700 dark:text-gray-400 mb-1.5">{{ __('lms.email') }} *</label>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-medium text-gray-700 dark:text-gray-400 mb-1.5">{{ __('lms.phone') }}</label>
                            ...
                            <div>
                                <label class="block text-xs font-medium text-gray-700 dark:text-gray-400 mb-1.5">{{ __('lms.registration_number') }}</label>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-medium text-gray-700 dark:text-gray-400 mb-1.5">{{ __('lms.title') }}</label>
                            ...
                            <div>
                                <label class="block text-xs font-medium text-gray-700 dark:text-gray-400 mb-1.5">{{ __('lms.department') }} *</label>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-medium text-gray-700 dark:text-gray-400 mb-1.5">{{ __('lms.hire_date_label') }}</label>
                            ...
                            <div class="flex items-end pb-2">
                                <label class="flex items-center gap-2 cursor-pointer">
                                    ...
                                    <span class="text-sm text-gray-700 dark:text-gray-300">{{ __('lms.active') }}</span>
                                </label>
                            </div>
                        </div>
                ...
                <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg z-10 border border-gray-200 dark:border-gray-700 modal-content-enter">
                    ...
                    <div class="px-6 py-5 border-b border-gray-200 dark:border-gray-700">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-base font-semibold text-gray-900 dark:text-white">{{ __('lms.view_detail') }}</h3>
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div class="ml-auto">
                                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold {{ $viewingUser->is_active ? 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400' : 'bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-400' }}">
                                    ...
                                    {{ $viewingUser->is_active ? __('lms.active') : __('lms.inactive') }}
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="px-6 py-4">
                        <dl class="space-y-3 text-sm">
                            <div class="flex justify-between py-1">
                                <dt class="text-gray-600 dark:text-gray-400">{{ __('lms.email') }}</dt>
                                <dd class="text-gray-900 dark:text-white font-medium">{{ $viewingUser->email }}</dd>
                            </div>
                            <div class="flex justify-between py-1">
                                <dt class="text-gray-600 dark:text-gray-400">{{ __('lms.phone') }}</dt>
                                <dd class="text-gray-900 dark:text-white">{{ $viewingUser->phone ?? '—' }}</dd>
                            </div>
                            <div class="flex justify-between py-1">
                                <dt class="text-gray-600 dark:text-gray-400">{{ __('lms.registration_number') }}</dt>
                                <dd class="text-gray-900 dark:text-white font-mono">{{ $viewingUser->registration_number ?? '—' }}</dd>
                            </div>
                            <div class="flex justify-between py-1">
                                <dt class="text-gray-600 dark:text-gray-400">{{ __('lms.department') }}</dt>
                                <dd class="text-gray-900 dark:text-white">{{ $viewingUser->department?->name ?? '—' }}</dd>
                            </div>
                            <div class="flex justify-between py-1">
                                <dt class="text-gray-600 dark:text-gray-400">{{ __('lms.hire_date') }}</dt>
                                <dd class="text-gray-900 dark:text-white">{{ $viewingUser->hire_date?->format('d.m.Y') ?? '—' }}</dd>
                            </div>
                            <div class="flex justify-between py-1">
                                <dt class="text-gray-600 dark:text-gray-400">{{ __('lms.last_login') }}</dt>
                                <dd class="text-gray-900 dark:text-white">{{ $viewingUser->last_login_at?->format('d.m.Y H:i') ?? __('lms.last_login_never') }}</dd>
                            </div>
                        </dl>
                    </div>
                    ...
                    <div class="grid grid-cols-4 gap-2 px-6 pb-4">
                        <div class="bg-primary-100 dark:bg-primary-900/30 rounded-lg p-3 text-center">
                            ...
                            <p class="text-[10px] text-primary-600 dark:text-primary-400 font-medium mt-0.5">{{ __('lms.total_enrollments') }}</p>
                        </div>
                        <div class="bg-emerald-100 dark:bg-emerald-900/30 rounded-lg p-3 text-center">
                            ...
                            <p class="text-[10px] text-emerald-600 dark:text-emerald-400 font-medium mt-0.5">{{ __('lms.completed') }}</p>
                        </div>
                        <div class="bg-red-100 dark:bg-red-900/30 rounded-lg p-3 text-center">
                            ...
                            <p class="text-[10px] text-red-600 dark:text-red-400 font-medium mt-0.5">{{ __('lms.failed') }}</p>
                        </div>
                        <div class="bg-amber-100 dark:bg-amber-900/30 rounded-lg p-3 text-center">
                            ...
                            <p class="text-[10px] text-amber-600 dark:text-amber-400 font-medium mt-0.5">{{ __('lms.in_progress') }}</p>
                        </div>
                    </div>
                    ...
                    @if($viewingUser->enrollments->count() > 0)
                        <div class="px-6 pb-4">
                            <h5 class="text-xs font-semibold text-gray-700 dark:text-gray-400 uppercase tracking-wider mb-2">{{ __('lms.courses') }}</h5>
            ...
            <div class="flex items-center justify-center min-h-screen px-4">
                ...
                <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-sm p-6 z-10 border border-gray-200 dark:border-gray-700 modal-content-enter">
                    <div class="text-center">
                        ...
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">{{ __('lms.staff_delete_title') }}</h3>
                        <p class="text-sm text-gray-600 dark:text-gray-400 mb-6">{{ __('lms.staff_delete_body') }}<br><span class="text-xs text-gray-500">Bu işlem geri alınabilir (soft delete).</span></p>
                ...
                <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md z-10 border border-gray-200 dark:border-gray-700 modal-content-enter">
                    ...
                    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                        <div class="flex items-center gap-3">
                            ...
                            <h3 class="text-base font-semibold text-gray-900 dark:text-white">{{ __('lms.import_excel') }}</h3>
                        </div>
            ...
            <div class="flex items-center justify-center min-h-screen px-4">
                ...
                <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-sm p-6 z-10 border border-gray-200 dark:border-gray-700 modal-content-enter">
                    <div class="text-center">
                        ...
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">{{ __('lms.staff_delete_title') }}</h3>
                        <p class="text-sm text-gray-600 dark:text-gray-400 mb-6">{{ __('lms.staff_delete_body') }}<br><span class="text-xs text-gray-500">Eğitim kaydı bulunan personeller atlanacaktır (soft delete).</span></p>
                        <div class="flex gap-3">
