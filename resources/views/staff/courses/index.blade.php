...
<div class="space-y-4 md:space-y-6">
    {{-- Filter Tabs --}}
    <div x-data="{ filter: 'all' }" class="space-y-3 md:space-y-4">
        <div class="overflow-x-auto -mx-4 px-4 md:mx-0 md:px-0">
            <div class="flex items-center gap-1 bg-gray-100 dark:bg-gray-700/50 rounded-xl p-1 w-max md:w-auto">
                @php
                    $filters = [
                        ...
                        'failed' => ['label' => 'Başarısız', 'count' => $enrollments->where('status', 'failed')->count()],
                    ];
                @endphp
                @foreach($filters as $key => $f)
                    <button @click="filter = '{{ $key }}'"
                        :class="filter === '{{ $key }}' ? 'bg-white dark:bg-gray-800 shadow-sm text-gray-800 dark:text-white' : 'text-gray-600 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'"
                        class="px-3 md:px-4 py-2 text-xs md:text-[13px] font-medium rounded-lg transition-all whitespace-nowrap">
                        {{ $f['label'] }}
                        <span class="ml-0.5 md:ml-1 text-[10px] md:text-xs opacity-60">({{ $f['count'] }})</span>
                    </button>
                @endforeach
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
            ...
            <div x-show="filter === 'all' || filter === '{{ $enrollment->status }}'" x-transition>
                <a href="{{ route('staff.courses.show', $enrollment->course_id) }}"
                   class="block rounded-xl border overflow-hidden transition-all group h-full
                   {{ $enrollment->status === 'failed'
                       ? 'bg-gray-100 dark:bg-gray-800/60 border-gray-300 dark:border-gray-600 opacity-80'
                       : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 hover:shadow-lg hover:border-primary-200 dark:hover:border-primary-800' }}">
                    {{-- Color Bar --}}
                    <div class="h-1.5" style="background-color: {{ $enrollment->status === 'failed' ? '#9CA3AF' : ($enrollment->course->category?->color ?? '#14B8A6') }}"></div>
                    ...
                    <div class="p-4 md:p-5">
                        ...
                        {{-- Title --}}
                        <h4 class="font-semibold text-gray-800 dark:text-white group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors line-clamp-2 mb-2">{{ $enrollment->course->title }}</h4>

                        {{-- Description --}}
                        @if($enrollment->course->description)
                            <p class="text-xs text-gray-600 dark:text-gray-400 line-clamp-2 mb-3">{{ $enrollment->course->description }}</p>
                        @endif
                        ...
                        @endphp
                        <div class="mt-3 pt-3 border-t {{ $enrollment->status === 'failed' ? 'border-gray-300 dark:border-gray-600' : 'border-gray-200 dark:border-gray-700' }}">
                            <div class="flex items-center gap-3 text-xs">
                                <span class="text-gray-500 dark:text-gray-400">
                                    Ön Sınav: <span class="font-semibold text-gray-700 dark:text-gray-300">{{ $preExam ? $preExam->score . '/100' : '—' }}</span>
                                </span>
                                <span class="text-gray-300 dark:text-gray-600">|</span>
                                <span class="text-gray-500 dark:text-gray-400">
                                    Son Sınav: <span class="font-semibold {{ $postExam && $postExam->is_passed ? 'text-emerald-600 dark:text-emerald-400' : ($postExam ? 'text-red-500 dark:text-red-400' : 'text-gray-700 dark:text-gray-300') }}">{{ $postExam ? $postExam->score . '/100' : '—' }}</span>
                                </span>
                            </div>
                            <div class="flex items-center justify-between mt-1.5">
                                <span class="text-xs text-gray-500 dark:text-gray-400">
                                    Deneme: {{ $currentAttempt }}/{{ $enrollment->course->max_attempts }}
                                </span>
                    ...
                    {{-- Action Footer --}}
                    <div class="px-4 md:px-5 py-3 border-t {{ $enrollment->status === 'failed' ? 'bg-gray-200/50 dark:bg-gray-700/20 border-gray-300 dark:border-gray-600' : 'bg-gray-50 dark:bg-gray-700/30 border-gray-200 dark:border-gray-700' }}">
                        <span class="text-xs font-medium {{ $enrollment->status === 'failed' ? 'text-gray-400 dark:text-gray-500' : 'text-primary-600 dark:text-primary-400 group-hover:underline' }}">
        ...
        @else
        <div class="bg-white dark:bg-gray-800 rounded-xl p-8 md:p-12 border border-gray-200 dark:border-gray-700 text-center">
            <div class="w-16 h-16 bg-gray-100 dark:bg-gray-700 rounded-2xl flex items-center justify-center mx-auto mb-4">
                <svg class="w-8 h-8 text-gray-300 dark:text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg>
            </div>
            <p class="text-gray-600 dark:text-gray-400 font-medium">Henüz atanmış bir eğitim bulunmuyor</p>
            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Eğitimler yöneticiniz tarafından atanacaktır.</p>
        </div>
        @endif
    </div>
</div>
